...
export function renderDiceConsequenceCard(dice) {
  if (!dice || typeof dice !== "object" || !dice.dice_required) return "";
  const success = Boolean(dice.success);
  const criticalSuccess = Boolean(dice.critical_success);
  const criticalFailure = Boolean(dice.critical_failure);
  ...
  return `
    <section class="gm-section-card gm-section-dice-consequence dice-result-card dice-result-${statusClass}">
      ...
      <dl class="dice-result-grid">
        ...
        <div><dt>Attribute Score</dt><dd>${escapeHtml(valueOrUnknown(dice.attribute_score ?? dice.base_attribute))}</dd></div>
        <div><dt>Difficulty</dt><dd>${escapeHtml(valueOrUnknown(dice.difficulty ?? dice.difficulty_modifier))}</dd></div>
        <div><dt>Final Target</dt><dd>${escapeHtml(valueOrUnknown(dice.final_target ?? dice.target))}</dd></div>
