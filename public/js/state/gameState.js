...
export function mergeParsedState(parsed) {
  ...
  return commitGameState(normalizeGameState(nextState));
}
...
export function normalizeGameState(state = {}) {
  ...
  const gojoAffinity = safeNumber(firstValue(source.gojo_affinity, source.gojoAffinity), 0);
  const nanamiAffinity = safeNumber(firstValue(source.nam_affinity, source.nanamiAffinity), 0);
