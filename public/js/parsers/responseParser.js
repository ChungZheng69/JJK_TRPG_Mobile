...
export function parseDifyAnswer(answer, apiPayload = {}) {
  ...
  const chosenFrontendState = findFrontendStateJson({
  ...
  if (debugStateJson) {
    mergeDebugJsonIntoState(state, debugStateJson, "frontend_state_json");
  } else {
    ...
    for (const candidate of candidates) {
      mergeCandidateIntoState(state, candidate.value, candidate.source, warnings);
    }

    parseVisibleTextIntoState(state, textWithoutJson);
  }
  ...
  const displayText = stripDebugJsonFromDisplay(textWithoutJson, allJsonBlocks).trim() || cleanedPlanning;
...
export function findFrontendStateJson({ rawText = "", jsonBlocks = [], inlineObjects = [], namedObjects = [], warnings = [] }) {
  ...
  return chosen
    ? { block: chosen.value, index: chosen.index, source: chosen.source, priority: chosen.priority }
    : null;
}
...
function frontendStatePriority(value) {
  ...
  if (value.game_state) return 400;
  if (Array.isArray(value.state_change_log) || Array.isArray(value.parser_warnings)) return 300;
  if (value.dice) return 200;
  if (value.mechanics || value.suggested_state_update) return 100;
  return 0;
}
...
function mergeDebugJsonIntoState(target, debugStateJson, source) {
  ...
  setField(target, "gojoAffinity", gameState.gojo_affinity, `${source}.game_state.gojo_affinity`);
  setField(target, "nanamiAffinity", gameState.nam_affinity, `${source}.game_state.nam_affinity`);
...
function mergeCandidateIntoState(target, candidate, source, warnings) {
  ...
  setFoundValue(target, "gojoAffinity", getFirstExisting(obj, FIELD_PATHS.gojoAffinity, source));
  setFoundValue(target, "nanamiAffinity", getFirstExisting(obj, FIELD_PATHS.nanamiAffinity, source));
