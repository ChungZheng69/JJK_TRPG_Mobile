...
function renderPanels() {
  ...
  els.panelGojo.textContent = valueOrUnknown(state.gojo_affinity ?? state.gojoAffinity);
  els.panelNanami.textContent = valueOrUnknown(state.nam_affinity ?? state.nanamiAffinity);
...
function applyLoadedSave(save) {
  ...
  for (const entry of chatHistory) {
    addMobileMessage(entry.role || "gm", entry.text || "", entry.rawJsonBlocks || []);
  }
  renderPanels();
}
