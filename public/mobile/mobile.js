import { formatApiError, sendChatMessage } from "../shared/api.js";
...
function renderPanels() {
  ...
  els.hp.textContent = formatResource(state.hp, state.maxHp);
  els.mp.textContent = formatResource(state.mp, state.maxMp);
  els.objective.textContent = valueOrUnknown(state.objective);
  els.panelHp.textContent = formatResource(state.hp, state.maxHp);
  els.panelMp.textContent = formatResource(state.mp, state.maxMp);
  ...
  renderTags(els.panelAttributes, formatAttributes(state.attributes, state.attributeMax));
